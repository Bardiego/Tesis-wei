# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 12:08:38 2020
...
def funcion_scrap_elcronista():
    
    diario_str = 'https://www.cronista.com'
    
    ctx = ssl.create_default_context()
    ctx.check_hostname = False
    ctx.verify_mode = ssl.CERT_NONE
    
    html = requests.get("https://www.cronista.com/").text
    ...
    for link in links:
        try:
            ...
            fecha_hora_publicacion = datetime.strptime(parser.find('span', {'class':'fecha'}).select_one('[content]')['content'], format)
            cuerpo_nota = parser.find('div', {'class':'article-container'})
            nota_tags = cuerpo_nota.findAll('p')
            nota = str()
            nota = nota.join(tag.text for tag in nota_tags)  
            nota = unicodedata.normalize("NFKD", nota)
            nota = nota.replace('\n','').replace('u200b', '').replace('\t', '')
    ...
    df.to_csv('ultimas_noticias_elcronista_full.txt', mode = 'a', index = False, header = False)
    df = pd.read_csv('ultimas_noticias_elcronista_full.txt', header=None)
    df[3] = df[3].apply(literal_eval)
    df = df.drop_duplicates(subset=1, keep='last')
    df = df.sort_values(by = 0, axis=0, ascending=False)
    df.to_csv('ultimas_noticias_elcronista_full.txt', mode = '+w', index = False, header = False)
